...
void run()
{
    ...
    OutEdgeIt e, eend;
    for(boost::tie(e, eend) = boost::out_edges(boost::vertex(src,G), G); e != eend; ++e) {
        s_flow_max += capacitymap[*e] - rescapacitymap[*e];
    }
    int flow_max = s_flow_max;

    
    int opt_cost = -(cost1-50*s_flow_max);
    bool increasing = true;

    int start = 1;
    //int end = min(b,s);
    int end = 1;
    int optimal = opt_cost;
    int position = 500;
    bool exp = true;
    //for( int i = min(b,s); i >= 1; i-- ) {
    while( start <= end ) {
    //for( int i = 1; i < min(b,s)+1; i++ ) {
        
        int current;
        if( exp ) {
            current = min(max(b,s)+1,2*end);
        } else {
            current = (start+end)/2;
        }
        ...
        //int t_cost = get_flow_cost(G,e_bnk,i,src,tgt,capacitymap,rescapacitymap);
        bool incr = increasing_flow_cost(G,e_bnk,current,src,tgt,capacitymap,rescapacitymap);
        //cout<<incr;
        ...
        if( end == max(b,s) ) exp = false;
        cout<<"start:"<<start<<" end:"<<end<<" current:"<<current<<endl;

        //opt_cost = max(opt_cost,t_cost);
    }
    int t_cost = get_flow_cost(G,e_bnk,position+1,src,tgt,capacitymap,rescapacitymap);
    cout<<t_cost<<endl;
